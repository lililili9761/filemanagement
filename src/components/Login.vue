<template>
  <div>
    ...
    >
      ...
      <div style="text-align: right;margin-top:5px;">还没注册？
        <router-link to="/register" @click.native="buttonstatus">马上注册</router-link>
      </div>
    </el-form>
  </div>
</template>
        ...
        if (valid) {
          ...
          this.$axios.post("/api/login", this.ruleForm).then(res => {
            ...
            if (this.existuser) {
              ...
              this.$cookies.set('username',this.ruleForm.account);
              this.$cookies.set('isadmin',this.isadmin);
              setTimeout(function() {
